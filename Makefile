...
test-profile-ratchet: ## enforce #335 duration ratchets from real lane profiles plus prior reports
	@test -d "$(TRACEFOLD_TEST_PROFILE_DIR)" || { echo "TRACEFOLD_TEST_PROFILE_DIR is required" >&2; exit 2; }
	@uv run python -m tests.support.profile aggregate \
		--profile-dir "$(TRACEFOLD_TEST_PROFILE_DIR)" \
		$(TEST_PROFILE_HISTORY_ARGS) --baseline tests/fixtures/issue_335_test_profile_baseline.json \
		--output artifacts/test-profile/ratchet-report.json \
		--enforce-ratchets
...
_trading-hard-cut-preflight-if-needed:
	@set -eu; \
		...
		case "$$migration_state" in \
			...
			20260829_0327\|t\|t) make --no-print-directory _trading-intent-quote-preflight ;; \
			20260829_0328\|t\|t) echo "Trading hard cuts are already present at database head 20260829_0328." ;; \
