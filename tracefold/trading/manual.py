...
ManualVenue = Literal["binance_usdm_live"]
_ManualRecordedVenue = Literal["binance_usdm_demo", "binance_usdm_live"]
_MONEY = Decimal("0.01")
_PRICE = Decimal("0.01")
_BPS = Decimal(10_000)
MAX_DEVELOPMENT_TEST_NOTIONAL_USD = Decimal("200")
...
class ManualTargetPickerState(StrEnum):
    PENDING = "PENDING"
    SENDING = "SENDING"
    SENT = "SENT"
    CONSUMED = "CONSUMED"
...
class ManualTargetPicker(_FrozenManualModel):
    """A durable, actor-bound Telegram choice surface created before a trade source exists."""

    picker_id: str = Field(pattern=r"^[0-9a-f-]{36}$")
    sources_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    sources: tuple[ManualTradeSource, ...] = Field(min_length=2, max_length=4)
    actor_user_id: int = Field(gt=0)
    chat_id: int
    source_message_id: int = Field(gt=0)
    interaction_message_id: int | None = Field(default=None, gt=0)
    reply_attempted_at_ms: int | None = Field(default=None, gt=0)
    selected_symbol: str | None = Field(default=None, pattern=r"^[A-Z0-9][A-Z0-9.-]{0,19}$")
    consumed_session_id: str | None = Field(default=None, pattern=r"^[0-9a-f-]{36}$")
    consumed_at_ms: int | None = Field(default=None, gt=0)
    state: ManualTargetPickerState
    created_at_ms: int = Field(gt=0)
    updated_at_ms: int = Field(gt=0)

    @model_validator(mode="after")
    def validate_picker_identity_and_state(self) -> ManualTargetPicker:
        symbols = tuple(source.base_symbol for source in self.sources)
        if len(set(symbols)) != len(symbols):
            raise ValueError("manual_target_picker_symbols_duplicate")
        identity = {
            (
                source.news_event_id,
                source.delivery_target_sha256,
                source.delivery_message_id,
                source.headline_zh,
                source.side,
                source.source_observed_at_ms,
            )
            for source in self.sources
        }
        if len(identity) != 1 or any(source.delivery_message_id != self.source_message_id for source in self.sources):
            raise ValueError("manual_target_picker_source_identity_mismatch")
        if self.sources_sha256 != canonical_sha256([source.model_dump(mode="json") for source in self.sources]):
            raise ValueError("manual_target_picker_sources_identity_invalid")
        if self.updated_at_ms < self.created_at_ms:
            raise ValueError("manual_target_picker_time_invalid")
        valid_shape = (
            (
                self.state is ManualTargetPickerState.PENDING
                and self.reply_attempted_at_ms is None
                and self.interaction_message_id is None
                and self.selected_symbol is None
                and self.consumed_session_id is None
                and self.consumed_at_ms is None
            )
            or (
                self.state is ManualTargetPickerState.SENDING
                and self.reply_attempted_at_ms is not None
                and self.interaction_message_id is None
                and self.selected_symbol is None
                and self.consumed_session_id is None
                and self.consumed_at_ms is None
            ...
            or (
                self.state is ManualTargetPickerState.SENT
                and self.reply_attempted_at_ms is not None
            ...
            or (
                self.state is ManualTargetPickerState.CONSUMED
                and self.reply_attempted_at_ms is not None
                and self.interaction_message_id is not None
                and self.selected_symbol in symbols
                and self.consumed_session_id is not None
                and self.consumed_at_ms is not None
                and self.consumed_at_ms >= self.reply_attempted_at_ms
            )
        )
        if not valid_shape:
            raise ValueError("manual_target_picker_state_shape_invalid")
        return self


class ManualAccountSnapshot(_FrozenManualModel):
    account_ref: str = Field(pattern=r"^[a-z0-9][a-z0-9._-]{0,63}$")
    venue: _ManualRecordedVenue
...
class ManualTradeIntent(_FrozenManualModel):
    ...
    def validate_confirmation_and_identity(self) -> ManualTradeIntent:
        ...
        if is_development_test_source(self.source) and self.selected.notional_usd > MAX_DEVELOPMENT_TEST_NOTIONAL_USD:
            raise ValueError("manual_development_test_notional_exceeds_cap")
...
def create_manual_trade_intent(
    *,
    ...
    values: dict[str, Any] = {
        ...
        "selected": selected,
        "reference_entry": _positive_decimal(reference_entry, "manual_trade_reference_entry_invalid"),
        "account_equity_usd": _positive_decimal(account_equity, "manual_trade_account_equity_invalid"),
        "guard": guard,
...
def is_development_test_source(source: ManualTradeSource) -> bool:
    return source.news_event_id.startswith("development-test:")
