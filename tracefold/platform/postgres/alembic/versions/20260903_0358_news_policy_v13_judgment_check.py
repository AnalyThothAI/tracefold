"""Open the News judgment CHECK to `news_triage_policy_v13` (#523).
...
- why_database_must_change: `news_verdicts_current_judgment_check` pins the model, OI and degraded branches
  to `news_triage_policy_v11` or `news_triage_policy_v12`. Policy v13 narrows two conditions inside
  `decide()` -- the deterministic listing guard no longer covers a frame the model marked
  `reader_value=none`, and the budget's reversal exemption compares against the newest *directional*
  delivered card rather than the newest card of any direction -- `TRIAGE_POLICY_VERSION` moves to v13, and
  every model, OI and degraded verdict is written under it, so without this revision the deployed workers
  cannot persist a single judgment. No withhold key, rule name or decision shape changes, so the rest of the
  predicate has nothing to say about v13.
- current_source_revision: 20260903_0357
- minimum_supported_source_revision: 20260903_0357
...
- archive_current_compatibility: compatible. Every row written under v11 or v12 keeps validating: the
  three branches now accept any of the three policy versions the `news_judgment_v2` contract has been
  written under, exactly as `0352` did for v11/v12. v11 and v12 judgments are audit truth of the previous
  epochs and are neither deleted nor rewritten; the worker never writes v12 again because
  `TRIAGE_POLICY_VERSION` is the only value it emits. `NOT VALID` was rejected because
  `news_verdicts.published_at_ms` is updated in place, and an update would re-check a v12 row against a
  v13-only predicate. Everything else in the predicate is byte-identical to `0352`.
- role_and_grant_impact: none; the single tracefold login is unchanged
- failure_state: the transaction rolls back completely and the v11/v12-only predicate stays
...
Revision ID: 20260903_0358
Revises: 20260903_0357
...
revision = "20260903_0358"
down_revision = "20260903_0357"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    # One CHECK holds all four judgment origins, so the whole predicate is restated from `20260903_0352`.
    # The liquidation branch is byte-identical to it; the model, OI and degraded branches change only their
    # policy-version literal, from `= ANY (ARRAY['news_triage_policy_v11', 'news_triage_policy_v12'])` to
    # `= ANY (ARRAY['news_triage_policy_v11', 'news_triage_policy_v12', 'news_triage_policy_v13'])`.
...
def downgrade() -> None:
    raise RuntimeError("news_policy_v13_judgment_check_forward_only")
