...
class LearningStorage:
    ...
    def taxonomy_candidate_registration(self, artifact_sha: str) -> dict[str, Any] | None:
        """Read the exact durable candidate-registration row used by taxonomy evaluation."""

        row = self.conn.execute(
            "SELECT kind, payload, created_at_ms FROM news_learning_artifacts WHERE artifact_sha = %s",
            (artifact_sha,),
        ).fetchone()
        return None if row is None else dict(row)

    def taxonomy_active_deployment(self) -> dict[str, Any] | None:
        """Latest Workers deployment receipt and the active Agent row it addresses."""
        ...
        row = self.conn.execute(
            ...
            SELECT active.artifact_sha AS active_agent_sha,
                   active.payload AS active_agent_payload,
                   deployment.artifact_sha AS deployment_receipt_sha,
                   deployment.parent_sha AS deployment_parent_sha,
                   deployment.payload AS deployment_payload,
                   manifest.manifest_sha,
                   manifest.stable_bundle_sha AS manifest_stable_bundle_sha,
                   manifest.candidate_shas AS manifest_candidate_shas,
                   manifest.image_digest AS manifest_image_digest,
                   manifest.runtime_revision AS manifest_runtime_revision,
                   manifest.registered_at_ms AS manifest_registered_at_ms
              FROM latest_active active
              LEFT JOIN LATERAL (
                SELECT artifact_sha, parent_sha, payload
                  FROM news_learning_artifacts
                 WHERE kind = 'deployment_receipt'
                   AND parent_sha = active.artifact_sha
                 ORDER BY created_at_ms DESC, artifact_sha DESC
                 LIMIT 1
              ) deployment ON TRUE
              LEFT JOIN news_agent_runtime_manifests manifest
                ON manifest.manifest_sha = active.payload ->> 'runtime_manifest_sha'
            """
        ).fetchone()
        return None if row is None else dict(row)

    def taxonomy_shadow_artifacts(self, artifact_shas: Sequence[str]) -> list[dict[str, Any]]:
        """Read the content-addressed shadow rows named by one evaluation document."""

        rows = self.conn.execute(
            "SELECT artifact_sha, kind, payload, created_at_ms FROM news_learning_artifacts "
            "WHERE artifact_sha = ANY(%s) ORDER BY artifact_sha",
            (list(artifact_shas),),
        ).fetchall()
        return [dict(row) for row in rows]

    def taxonomy_regression_sources(self, artifact_shas: Sequence[str]) -> list[dict[str, Any]]:
        """Release reports, candidates and datasets behind taxonomy regression references."""

        rows = self.conn.execute(
            """
            SELECT release.artifact_sha AS evidence_sha,
                   release.parent_sha AS evidence_parent_sha,
                   release.payload AS evidence_payload,
                   report.artifact_sha AS report_sha,
                   report.parent_sha AS report_parent_sha,
                   report.payload AS report_payload,
                   candidate.artifact_sha AS candidate_artifact_sha,
                   candidate.parent_sha AS candidate_parent_sha,
                   candidate.payload AS candidate_payload,
                   development.artifact_sha AS development_artifact_sha,
                   development.payload AS development_payload,
                   validation.artifact_sha AS validation_artifact_sha,
                   validation.payload AS validation_payload
              FROM news_learning_artifacts release
              JOIN news_learning_artifacts report
                ON report.kind = 'evaluation_report'
               AND report.artifact_sha = release.parent_sha
              LEFT JOIN news_learning_artifacts candidate
                ON candidate.kind = 'candidate'
               AND candidate.payload ->> 'candidate_sha' = release.payload ->> 'candidate_sha'
              LEFT JOIN news_learning_artifacts development
                ON development.kind = 'dataset'
               AND development.artifact_sha = report.payload #>> '{evidence,development_dataset_sha}'
              LEFT JOIN news_learning_artifacts validation
                ON validation.kind = 'dataset'
               AND validation.artifact_sha = report.payload #>> '{evidence,validation_dataset_sha}'
             WHERE release.kind = 'release_evidence'
               AND release.artifact_sha = ANY(%s)
             ORDER BY release.artifact_sha
            """,
            (list(artifact_shas),),
        ).fetchall()
        return [dict(row) for row in rows]
    ...
    def taxonomy_gold_sources(self, acceptance_ids: Sequence[str]) -> list[dict[str, Any]]:
        ...
        rows = self.conn.execute(
            """
            SELECT a.review_id AS acceptance_id,
                   a.accepts_review_id,
                   a.release_eligible AS acceptance_release_eligible,
                   a.created_at_ms AS accepted_at_ms,
                   j.review_id,
                   j.event_id,
                   j.rubric_version,
                   j.reviewer,
                   j.payload,
                   j.release_eligible AS judgment_release_eligible,
                   j.evidence_version,
                   j.should_push,
                   j.dimensions,
                   j.novelty,
                   j.expected_correction,
                   j.selection,
                   primary_j.payload AS primary_payload,
                   source.evidence_sha256,
                   source.evidence_snapshot,
                   source.evidence_release_eligible,
                   source.opened_at_ms,
                   source.ingest_mode,
                   source.verdict,
              ...
              LEFT JOIN news_review_records_v1 primary_j
                ON primary_j.review_id = j.payload #>> '{taxonomy_review,adjudicates_review_id}'
             WHERE a.review_kind = 'acceptance'
               AND a.subject_kind = 'event'
               AND j.review_kind = 'judgment'
               AND a.review_id = ANY(%s)
