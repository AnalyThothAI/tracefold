"""Bounded Binance USD-M live adapter for the manual execution authority.

The production base URL is deliberately not configurable. Every provider
...
class BinanceManualClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._client = httpx.Client(
            base_url=BINANCE_USDM_LIVE_BASE_URL,
    ...
    def account(self) -> ManualVenueAccount:
        ...
        account_config = self._signed("GET", "/fapi/v1/accountConfig")
        try:
            equity = _nonnegative_decimal(payload.get("totalMarginBalance"), "binance_manual_account_invalid")
            can_trade = account_config.get("canTrade")
    ...
    def position(self, symbol: str) -> ManualVenuePosition:
        ...
        config_payload = self._signed_sequence(
            "GET",
            "/fapi/v1/symbolConfig",
            {"symbol": normalized_symbol},
        )
        config_rows = [
            row for row in config_payload if isinstance(row, Mapping) and row.get("symbol") == normalized_symbol
        ]
        if len(config_rows) != 1:
            raise ManualVenueError("binance_manual_position_invalid", retryable=True)
        try:
            leverage = int(config_rows[0].get("leverage"))
        ...
        if not rows:
            mark_payload = self._public("GET", "/fapi/v1/premiumIndex", {"symbol": normalized_symbol})
            if mark_payload.get("symbol") != normalized_symbol:
                raise ManualVenueError("binance_manual_position_invalid", retryable=True)
            return ManualVenuePosition(
                ...
                leverage=leverage,
                mark_price=_positive_decimal(mark_payload.get("markPrice"), "binance_manual_position_invalid"),
                unrealized_pnl_usd=Decimal("0"),
                liquidation_price=Decimal("0"),
            )
        if len(rows) != 1 or rows[0].get("positionSide") not in {None, "BOTH"}:
            raise ManualVenueError("binance_manual_position_invalid", retryable=True)
        row = rows[0]
        try:
            return ManualVenuePosition(
                ...
                leverage=leverage,
                mark_price=_optional_nonnegative_decimal(row.get("markPrice")),
                unrealized_pnl_usd=_optional_decimal_signed(row.get("unRealizedProfit")),
                liquidation_price=_optional_nonnegative_decimal(row.get("liquidationPrice")),
            )
    ...
    def cancel_leg(self, plan: ManualExecutionPlan, leg: Literal["take_profit", "stop_loss"]) -> bool:
        client_id = getattr(plan, f"{leg}_client_order_id")
        current = self.query_algo_order(client_algo_id=client_id)
        if current is None or current.status.upper() in {
            "CANCELED",
            "CANCELLED",
            "FINISHED",
            "EXPIRED",
        ...
        if (
            str(payload.get("clientAlgoId") or "") == client_id
            and payload.get("algoId") is not None
            and str(payload.get("code") or "") == "200"
        ):
            return True
        raise ManualVenueError("binance_manual_algo_cancel_response_invalid", ambiguous=True)
    ...
    def _public(
        ...
        query = urlencode([(key, _parameter(value)) for key, value in (params or {}).items()]) or None
        payload = self._request_json(method, path, content=None, query=query, write=False)
...
def _optional_decimal_signed(value: object) -> Decimal | None:
    if value is None:
        return None
    try:
        parsed = Decimal(str(value))
...
def _optional_nonnegative_decimal(value: object) -> Decimal | None:
    parsed = _optional_decimal_signed(value)
