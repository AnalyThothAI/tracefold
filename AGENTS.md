...
- Inspect the checkout with available repository/status tools; if unavailable, use `git rev-parse --show-toplevel`, `git status --short --branch`, and `git worktree list`. For edits, keep the assigned task worktree or prefer the app's **Worktree** mode; use `git worktree add` when app worktree creation is unavailable.
...
- Code review belongs to the PR; the local `code-review` skill is not a final checkpoint. Use an available security review when the changed surface affects a security boundary, including authentication, authorization, secrets, or capital/order authority.
