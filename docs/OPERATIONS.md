...
expression index with the STORED generated fingerprint, preserving every
fact/current payload with no dual path. Keep Serve and Workers stopped through
the current head. Migration `20260813_0264` keeps the source-time partial
...
and full heap visibility before restoring the single writer. Separately, once
the News Push writer is running, observe its
