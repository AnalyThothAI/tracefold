...
manual Binance Demo key/secret files, and the PostgreSQL role password files.
...
Create a Binance USD-M Demo API key on a dedicated manual Demo account, with
Read and Futures Trade only, and store the pair in regular non-symlink files:

```bash
install -m 600 /dev/null ~/.tracefold/binance_manual_demo_api_key
install -m 600 /dev/null ~/.tracefold/binance_manual_demo_api_secret
# Populate the two files without printing their contents, then verify only redacted status:
uv run tracefold config
```

Do not use the automatic account or its `binance_demo_api_*` files. The manual
files are not generated by `tracefold init` and are mounted only into the
manual executor. Configure the operator user ID (not the channel ID) and typed
risk presets under the existing `trading` block:
...
`trading.manual.interaction_available=true`; output remains redacted. From the
deployment-clean primary checkout, `make up` migrates the database, starts one
manual executor, and enables the Workers callback loop. The executor is locked
to `https://demo-fapi.binance.com`; this configuration cannot select Binance
live, another CEX, or an on-chain signer.
