...
5. Run focused development checks and the completion checks named by the root task matrix inside the task worktree. `make test-evidence` is the complete local preflight when the full set is required. Do not use the primary checkout's shared runtime stack as task-test evidence; fixed CI reruns every owner for the pushed SHA.
