...
| `shared/ui/`             | Reusable presentational primitives (`Card`, `Metric`, `Bar`, `FactGrid`, `KeyValue`, `ActionButton`, `IconButton`, `PageState`, `Drawer`, `RouteBackLink`, `PageShell`, `SourceLine`, `EmptyNote`). No server fetching. |
...
- **Shared primitives.** `Card`, `Metric`, `Bar`, `FactGrid`, `KeyValue`,
  `ActionButton`, `IconButton`, `PageState`, `Drawer`, `PageShell`, `SourceLine` and
  `EmptyNote` in `shared/ui` own
  the console's panel, figure, proportion, labelled-fact, key/value, button,
  page-state, sheet, route-measure, provenance-line and nothing-here shapes. A feature may frame a primitive with its own class but must not
