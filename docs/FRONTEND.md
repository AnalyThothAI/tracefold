...
- **Topbar search.** The single topbar search box is News-only: its label is
  `news search`. On `/news` and `/news/oi` its approved placeholder is `事件 / base_symbol / 场所` and
