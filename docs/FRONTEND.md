...
URL-owned feed state), `state/` (`useAnchoredEventFeed`, `useFeedCursor`), and
...
- **News routes.** `/news` is a decision-first scan surface over the flat
  ...
  *actual* returns only, told apart from the model's judgment by weight — the
  ...
  pill (`流水线注意/异常` from `health.overall`, the failing item's
  `summary_zh`, linking to `/news/status`) and a 24 h funnel card
  (`收到 / 送审 / 符号落表 / 决定推送 / 已送达` from `funnel_24h` as five
  `Metric` tiles, plus one conversion sentence). A healthy pipeline shows no
  pill at all: the sidebar already carries a dot on the status destination and
  the status route carries the full read, and a permanent green `正常` beside a
  feed is a light the reader learns to stop seeing.
  ...
  contract the shell and feed implement between them. No other binding fires
  while a text field has focus, and none of the rest uses a modifier. Counts
  and key hints are `aria-hidden`: a number that changes every three seconds
  must never rename a tab or a link.
...
- **Shared primitives.** `Card`, `Metric`, `Bar`, `FactGrid`, `KeyValue`,
  `ActionButton`, `IconButton`, `PageState`, `Toast`, `Drawer`, `CommandPalette` and
  `ShortcutsDialog` in `shared/ui` own the console's panel, figure, proportion,
  labelled-fact, key/value, button, page-state, confirmation, sheet, palette and
  keyboard-map shapes. A feature may frame a primitive with its own class but must not
  ...
  Radix is used directly, and only for the three things that need a focus trap or a
  dismiss layer: `ShortcutsDialog`, `Drawer` and `CommandPalette`.
...
7. At tablet width around `834px`, confirm the desktop sidebar is not mounted, the topbar trigger opens the drawer, drawer route navigation and topbar search still work, and the News list and no-overflow contract remain intact.
   ...
   the funnel's `收到`.
8. Exercise the keyboard on `/news`: `⌘K` opens and closes the palette (and
   opens it from inside the search box), `J`/`K` move the cursor and scroll it
...
9. From a row at `≥1024px`, confirm a plain click opens the drawer with the
   ...
   Event without closing it, and `打开整页` reaches
