...
export function TradingAnalysisDetail({ item, token }: { item: TradingCase; token: string }) {
  ...
  return (
    <section aria-label={`分析案例 ${item.base_symbol}`} className="trading-case-detail">
      <header className="trading-analysis-hero">
        <span>FROZEN DECISION · {caseClock(item.decided_at_ms)}</span>
        <h2>{item.base_symbol}</h2>
        <div className="trading-analysis-verdict">
          <strong>{caseVerdict(item)}</strong>
          <small>
            {item.analysis_decision?.action ??
              item.analysis_action ??
              item.analysis_status ??
              item.state}
          </small>
        </div>
        <p>{word(decision?.reason ?? decision?.research_notes)}</p>
        <p>
          发布状态：
          {item.analysis_decision?.publish_status ?? item.analysis_publish_status ?? "未取得"}
        </p>
        {item.source_item_id ? (
          <Link
        ...
        <Link
          className="trading-analysis-executions"
          to={withResearchReturn(
            `/trading?tab=executions&execution_case=${encodeURIComponent(item.case_id)}`,
            researchFrom,
          )}
        >
          查看关联执行
      ...
      <details className="trading-analysis-disclosure">
        <summary>冻结判定字段与来源身份</summary>
        <div>
          <Card
            flush
            title="Agent 判断"
      ...
      <Card flush title="复核状态" hint={reviewMode}>
        {watch ? (
          <div className="trading-case-checks">
            ...
            {watch.child_case_id ? (
              <p>
                <Link to={withResearchReturn(`/trading?case=${watch.child_case_id}`, researchFrom)}>
                  查看条件命中的子 Case
                </Link>
              </p>
            ) : null}
          ...
          <div className="trading-case-checks">
            ...
            {item.root_chain.map((member) => (
              <p key={member.case_id}>
                <Link to={withResearchReturn(`/trading?case=${member.case_id}`, researchFrom)}>
            ...
            {item.analysis_attempts?.length ? (
              item.analysis_attempts.map((attempt) => (
                <div className="trading-case-checks" key={attempt.claim_attempt}>
                  ...
                  <p>
                    用量：输入 {word(attempt.input_tokens)} / 输出 {word(attempt.output_tokens)}{" "}
                    token · 费用{" "}
                  ...
                  {(attempt.physical_calls ?? []).map((call) => (
                    <p key={call.call_index}>
                      ...
                      {word(call.timeout_ms)} ms · 本地开始 {attemptClock(call.started_at_ms)} ·
