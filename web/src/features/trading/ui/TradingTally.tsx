 ...
 * (#680): exit minus entry notional, signed by direction, less every commission the venue charged.
 * Venue funding is included only after complete coverage. A closed plan with missing inputs is counted as missing, and
    ...
    >
      ...
      <div className="trading-fact-grid" data-columns="4">
        <span className="trading-fact" data-tone={moneyTone(today)}>
          ...
          <small>
            {totals
              ? `平仓 ${totals.closed_today} · 已知 ${totals.net_known_today} · 缺失 ${totals.net_missing_today}`
              : "读自 totals"}
          </small>
        </span>
        <span className="trading-fact" data-tone={moneyTone(total)}>
          ...
          <small>
            {totals
              ? `平仓 ${totals.closed_total} · 已知 ${totals.net_known_total} · 缺失 ${totals.net_missing_total}`
              : "读自 totals"}
          </small>
        </span>
