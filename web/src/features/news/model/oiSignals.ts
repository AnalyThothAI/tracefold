 ...
 *
 * Nothing here judges. Every rule name and measurement on this page is a server value read back from the
 * trace `oi_judgment_trace()` wrote; this module turns integers into characters and picks which Chinese
 * word names a key the server already chose. Re-deriving any of it from `leader_title` would be
 * `oi_signal_parser_v1` running a second time in the browser, drifting from the judge the moment either
 * side changed.
 */
 ...
 *
 * There are two of them since #458. The lane judges the provider's *format* and nothing else, so a frame
 * either parsed and was stored or it did not parse. The four push-gate names it used to have — and the
 * `pushed` / `withheld` tabs that grouped them — went with the notification rule.
 */
export const OI_STORED_RULE = "stored";
export const OI_PARSE_FAILED_RULE = "oi_parse_failed";

const OI_RULE_ZH: Record<string, string> = {
  [OI_STORED_RULE]: "已解析入账（推送由 Signal 通道负责）",
...
/**
 * Each tab's count. Two server fields, and they do not count the same thing on purpose.
 *
 * `parse_failed` comes from `oi.by_rule_24h`, keyed on the judged rule. `all` cannot: that bucket holds
 * *judged verdicts*, and a frame that arrived and has not been judged yet renders as a row — with no `oi`
 * block — while belonging to none of them, so summing them would advertise fewer than the tab can reach.
 * Nor is `all` `telemetry_received_24h`, which counts provider items *before* the Gate and so names frames
 * no row can ever show. It is `telemetry_events_24h`: Events on this admission, which is the table's own
 * universe. `received` keeps its tile in the band above, where it is the lane's intake.
 */
export function oiTabCount(
  ...
  return byRule[OI_PARSE_FAILED_RULE] ?? 0;
}
 ...
 *
 * This is an annotation, not a gate: News only stores this frame, and whether Alpha would have opened a
 * Case for it is a separate question with separate numbers. `positive` marks
