...
export function useShellChromeData(session: AppSession): ShellChromeData {
   ...
   * `Esc` back to the feed. The reading cursor (`J`/`K`/`Enter`/`Space`) belongs to the feed, and digits
   * to its task tabs.
   */
 ...
 * a destination, a task tab, or a symbol filter — collapsed into one keystroke. Review writes happen only
 * through the evidence-bound ReviewDesk surface.
 *
