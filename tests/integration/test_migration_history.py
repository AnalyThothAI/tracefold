...
def test_migration_tree_is_one_root_and_head_in_the_flat_package() -> None:
    ...
    assert revisions[0].down_revision == "20260901_0344"
    assert revisions[1].down_revision == "20260901_0343"
    assert revisions[2].down_revision == "20260901_0342"
    assert revisions[3].down_revision == "20260901_0341"
    assert revisions[4].down_revision == BASELINE
...
def test_current_head_downgrade_is_irreversible() -> None:
    ...
    with pytest.raises(RuntimeError, match="trading_runtime_exposure_projection_forward_only"):
