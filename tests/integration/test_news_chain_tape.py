...
from tracefold.news.chain_tape.evm import address_topic, normalize_address, topic_address
...
def test_a_restart_resumes_from_the_durable_position_and_the_overlap_adds_nothing(conn: Any) -> None:
    ...
    assert after_first["high_water_block"] == BUY_BLOCK
    ...
    # The overlap really was re-read: the window starts behind the recorded position.
    assert chain.log_calls[-1][0] == BUY_BLOCK - 30
...
def test_a_wide_backlog_is_walked_in_bounded_ranges_rather_than_one_request(conn: Any) -> None:
    ...
    assert [to_block - from_block for from_block, to_block in chain.log_calls] == [100_030] * 6
    state = _state(conn)
    assert state is not None
    assert state["high_water_block"] == SELL_BLOCK + 300_000
...
def test_retention_deletes_by_block_time_and_leaves_everything_inside_the_window(conn: Any) -> None:
    ...
    conn.commit()
    janitor = JanitorLoop(db=_Db(conn), cold_db=_Db(conn), bus=None, retention_chain_tape_days=90)
...
def test_the_recorded_wallet_topics_are_what_the_log_filter_carries() -> None:
