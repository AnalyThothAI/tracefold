...
def test_a_stopped_out_signal_is_one_closed_row_with_its_exit_price_and_realized_result(
    ...
    rows = [row for row in data["executions"] if row["signal_id"] == _SIGNAL_ID]
...
def test_a_flattened_signal_names_the_operator_exit_and_its_command_stays_recorded(
    ...
    data = _executions(tmp_path)
    row = next(item for item in data["executions"] if item["signal_id"] == _SIGNAL_ID)
    assert row["stage"] == "closed"
...
def _flatten_intent():
    from tracefold.trading.storage.execution_stream import prepare_operator_intent

    return prepare_operator_intent(
        command_id=_FLATTEN_COMMAND_ID,
