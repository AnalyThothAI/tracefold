...
def test_execution_stream_role_privileges_are_minimal() -> None:
    tables = (
    ...
    sequences = (
        "trading_trade_signals_seq_seq",
        "trading_operator_intents_seq_seq",
        "trading_execution_observations_seq_seq",
    )
    conn = connect_postgres_test(read_only=False)
    try:
        acl = {
            (row["role_name"], row["object_name"], row["privilege"])
            for row in conn.execute(
                """
                SELECT role.rolname AS role_name, object.relname AS object_name,
                       lower(acl.privilege_type) AS privilege
                  FROM pg_class object
                  CROSS JOIN LATERAL aclexplode(object.relacl) acl
                  JOIN pg_roles role ON role.oid = acl.grantee
                 WHERE object.relname = ANY(%s)
                   AND role.rolname = ANY(%s)
                   AND NOT acl.is_grantable
                """,
                ([*tables, *sequences], ["tracefold_serve", "tracefold_workers", "tracefold_nautilus"]),
            ).fetchall()
        }
        stream_boundary = runtime_role_contract(conn)["checks"]["execution_stream_boundary"]
    finally:
        conn.close()

    expected_acl = {
        *(("tracefold_serve", table, "select") for table in tables),
        *(("tracefold_workers", table, "select") for table in tables),
        *(("tracefold_nautilus", table, "select") for table in tables),
        *(("tracefold_workers", table, "insert") for table in tables if table != "trading_execution_observations"),
        ("tracefold_nautilus", "trading_execution_observations", "insert"),
        *(
            ("tracefold_workers", sequence, privilege)
            for sequence in sequences[:2]
            for privilege in ("select", "usage")
        ),
        *(
            ("tracefold_nautilus", sequence, privilege)
            for sequence in sequences[2:]
            for privilege in ("select", "usage")
        ),
    }
    assert acl == expected_acl
    assert stream_boundary is True


def test_execution_stream_roles_can_only_use_their_owned_append_path() -> None:
    signal = _prepare_signal(suffix="a")
    activation = ExecutionProfileActivation(
        runtime_profile_id="demo-v1",
        account_slot="binance_usdm_primary",
    ...
    observation_batch = prepare_execution_observations(
        (
            _observation(
                event="f",
                signal_id=signal.value.signal_id,
    ...
    worker = connect_postgres_test(read_only=False)
    nautilus = connect_postgres_test(read_only=False)
    serve = connect_postgres_test(read_only=False)
    try:
        worker.execute("SET ROLE tracefold_workers")
        worker_repo = TradingRepository(worker)
        with worker.transaction():
            worker_repo.append_trade_signal(signal)
            worker_repo.append_execution_profile_activation(activation)

        nautilus.execute("SET ROLE tracefold_nautilus")
        with nautilus.transaction():
            TradingRepository(nautilus).append_execution_observations(observation_batch)

        serve.execute("SET ROLE tracefold_serve")
        assert serve.execute("SELECT count(*) AS n FROM trading_trade_signals").fetchone()["n"] == 1
        with pytest.raises(psycopg.errors.InsufficientPrivilege):
            serve.execute("TRUNCATE trading_trade_signals")
        serve.rollback()
        ...
        with pytest.raises(psycopg.errors.InsufficientPrivilege):
            worker.execute(
                ...
                INSERT INTO trading_execution_observations (
                  ...
                  normalized_kind, occurred_at_ns, observed_at_ns,
                  native_identity_references, summary, payload_digest, payload
                ) VALUES ('0', 'demo-v1', 'r', 's', 'risk', 1, 1, '[]', '{}', '0', '{}')
                """
            )
        worker.rollback()
        ...
        with pytest.raises(psycopg.errors.InsufficientPrivilege):
            nautilus.execute(
                ...
                ) VALUES ('0', 'case', '0', 'market', 'long', 1, 2, '0', '{}', '{}')
                """
            )
        nautilus.rollback()
    finally:
