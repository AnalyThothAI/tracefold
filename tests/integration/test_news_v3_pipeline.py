...
def test_analyst_evidence_bundle_reads_macro_module_current_from_real_schema(conn) -> None:
    """The Analyst bundle used to select columns that do not exist on macro_module_current (module_key/health/
    headline) and crashed every escalation into the DLQ; this pins the projection to the real schema."""

    from tracefold.news.analyst_evidence import build_evidence_bundle

    repos = repositories_for_connection(conn)
    row = conn.execute(
        "SELECT event_id, opened_at_ms FROM news_events WHERE admission='candidate' ORDER BY opened_at_ms LIMIT 1"
    ).fetchone()
    assert row is not None
    now_ms = int(row["opened_at_ms"]) + 120_000
    with repos.transaction():
        conn.execute("DELETE FROM macro_module_current WHERE module_id = 'rates_fed'")
        conn.execute(
            """
            INSERT INTO macro_module_current
              (module_id, fact_cutoff_ms, payload_json, payload_hash, updated_at_ms,
               current_health_state, history_depth_state)
            VALUES ('rates_fed', %s, %s::jsonb, 'h1', %s, 'current', 'partial')
            """,
            (
                now_ms - 1000,
                json.dumps(
                    {
                        ...
                        "label": "利率与美联储",
                        "summary": {"headline": "10Y 4.75%"},
                    }
                ),
                now_ms,
            ),
        )
        if repos.news.latest_verdict(event_id=row["event_id"], stage="triage") is None:
    ...
    modules = repos.news.macro_state()
    rates = next(m for m in modules if m["module_id"] == "rates_fed")
    assert rates["label"] == "利率与美联储"
    assert rates["current_health_state"] == "current"
    assert rates["history_depth_state"] == "partial"
    assert rates["headline"] == "10Y 4.75%"
    assert set(rates) == {
        "module_id",
        "label",
    ...
    bundle = build_evidence_bundle(repos, event_id=row["event_id"], now_ms=now_ms, queue_lag_ms=0)
    assert bundle is not None
    macro = bundle.payload["macro"]
    assert [m["module_id"] for m in macro["modules"]] == ["rates_fed"]
