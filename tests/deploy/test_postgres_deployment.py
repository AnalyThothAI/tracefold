...
def test_runtime_role_migration_validates_the_current_role_contract() -> None:
...
def test_compose_mounts_only_role_credentials_into_steady_runtimes() -> None:
    ...
    assert "${HOME}/.tracefold/binance_demo_api_key:/root/.tracefold/binance_demo_api_key:ro" in worker_volumes
    assert "${HOME}/.tracefold/binance_demo_api_secret:/root/.tracefold/binance_demo_api_secret:ro" in worker_volumes
    ...
    for service_name, service in compose["services"].items():
        if service_name not in {"workers", "nautilus"}:
            assert not any(
                "binance_demo_api_key" in volume
                or "binance_demo_api_secret" in volume
                or "hyperliquid_private_key" in volume
                for volume in service.get("volumes", [])
            )
    assert not any(
        "hyperliquid_private_key" in volume
