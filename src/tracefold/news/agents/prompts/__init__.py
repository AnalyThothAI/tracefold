...
def analyst_system_prompt() -> str:
    memory = (_HERE / "NEWS_ANALYST.md").read_text(encoding="utf-8")
    return (
        "你是 Tracefold News Analyst。严格按下面的领域记忆工作，只依据工具返回的数据，输出结构化 AnalystVerdict。\n\n"
        + memory
    )


__all__ = ["TRIAGE_SYSTEM_PROMPT", "analyst_system_prompt"]
