...
class _ProgressionReviewSignature(dspy.Signature):  # type: ignore[misc]
    evidence_json: str = dspy.InputField(desc="Bounded current item and previously delivered candidates")
    review: ProgressionReviewAnswer = dspy.OutputField(desc="One exact relationship verdict and Chinese reason")
...
class DspyProgressionVerifier:
    ...
    def __init__(
        ...
        *,
        ...
        self._predict = dspy.Predict(
            _ProgressionReviewSignature.with_instructions(_INSTRUCTION),
            temperature=0,
            max_tokens=PROGRESSION_REVIEW_MAX_TOKENS,
        )
    ...
    async def review(
        ...
        *,
        ...
        response = await self._adapter.invoke(request, self._predict)
